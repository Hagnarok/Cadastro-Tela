...
def baixar_arquivo(nome_arquivo):
    try:
        url_arquivo = URL_REPOSITORIO + nome_arquivo
...
def atualizar_programa():
    if verificar_atualizacao():
        # Baixar o novo arquivo Python (ou outros arquivos que deseja atualizar)
        baixar_arquivo('codigo_cadastro.py')

        # Atualizar a versão local após o download
        with open(arquivo_versao_local, 'w') as f:
...
root = tk.Tk()
root.title("Cadastro de Telas - Programa desenvolvido por Gustavo Henrique de Araujo Alves")
