...
import psutil  # Biblioteca para manipular processos
import time
import stat  # Biblioteca para manipulação de permissões de arquivo
import hashlib
...
# Caminho completo para o EXE local
caminho_exe_local = os.path.join(pasta_documentos, 'Cadastro de Telas.exe')
...
# Função para encontrar o caminho do script atualizador, seja no ambiente empacotado ou de desenvolvimento
def encontrar_caminho_atualizador():
    # Verifica se o programa está rodando como um EXE empacotado
    if hasattr(sys, '_MEIPASS'):
        return os.path.join(sys._MEIPASS, "atualizador.py")
    else:
        return os.path.join(os.path.expanduser("~"), "Desktop", "atualizador.py")
...
def substituir_exe():
    try:
        ...
        # Caminho correto do EXE na Área de Trabalho
        caminho_exe_area_trabalho = os.path.join(os.path.expanduser("~"), "Desktop", nome_exe)
        caminho_exe_novo = os.path.join(os.path.expanduser("~"), "Desktop", nome_exe_novo)

        # Fazer o download do novo EXE
        resposta = requests.get(URL_EXE, stream=True)

        if resposta.status_code == 200:
            # Salvar o novo EXE na Área de Trabalho com nome temporário
            ...
            caminho_atualizador = encontrar_caminho_atualizador()
            os.startfile(caminho_atualizador)
...
def verificar_atualizacao():
    try:
        ...
        if resposta.status_code == 200:
            versao_remota = resposta.text.strip()  # Ler versão remota
...
def atualizar_programa():
    if verificar_atualizacao():
        # Atualizar a versão local com a versão remota após o download bem-sucedido
        try:
            versao_remota = requests.get(URL_REPOSITORIO + 'versao.txt').text.strip()
            with open(arquivo_versao_local, 'w') as f:
                f.write(versao_remota)  # Atualizar o arquivo local 'versao.txt'
